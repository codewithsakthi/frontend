...
const groupSemesterTrendFromMarks = (marks = []) => {
  const grouped = filterGradedSubjects(marks).reduce((acc, item) => {
    const semester = Number(item.semester || 0);
    if (!semester) return acc;
    ...
    const gp = gradePointFor(item);
    if (gp > 0) {
      acc[semester].gradePoints.push(gp);
    }
    if (item.internal_marks !== null && item.internal_marks !== undefined && Number.isFinite(Number(item.internal_marks))) {
      acc[semester].internals.push(Number(item.internal_marks));
    }
    if (FAIL_GRADES.has(normalizeGrade(item.grade))) {
      acc[semester].failures += 1;
  ...
  return Object.entries(grouped)
    ...
    .filter((item) => item.averageGradePoints > 0 || item.averageInternal > 0)
    .sort((a, b) => a.semester - b.semester);
};
...
export const buildStudentIntelligence = (student, record) => {
  ...
  const backendSemesterTrend = (record?.semester_performance || [])
    .filter(item => item.average_grade_points !== null && item.average_grade_points !== undefined && Number(item.average_grade_points) > 0)
    .map(item => ({
      ...item,
  ...
  const semesterTrend = backendSemesterTrend.length
    ? backendSemesterTrend
    : groupSemesterTrendFromMarks(record?.marks || record?.semester_grades || []);
